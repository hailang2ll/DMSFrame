...
namespace DMST.BizLogic.WebServices
{
    public class WebServiceFrameBase : WebServiceBase
    {
        protected void Log(int userId, string message, Exception exception)
        {
            ...
            if (exception == null)
            {
                Sys_LogManager.InsertLogInfo(EnumSysSubSysIDType.AdminService, EnumSysLogType.Adm_Admin, userId, message);
            }
            else
            {
        ...
        public bool CheckRight(string strRightName, bool writeLogFlag, ref BaseResult result)
        {
            ...
            if (!resultFlag)
            {
                ...
                if (writeLogFlag)
                {
                    Sys_LogManager.InsertLogInfo(EnumSysSubSysIDType.AdminService, EnumSysLogType.Adm_Admin, userTicket.UserID, result.errmsg);
        ...
        public bool CheckRightID(string strRightID, ref BaseResult result)
        {
            ...
            if (!resultFlag)
            {
                ...
                Sys_LogManager.InsertLogInfo(EnumSysSubSysIDType.AdminService, EnumSysLogType.Adm_Admin, 0, result.errmsg);
