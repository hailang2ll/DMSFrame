...
namespace WDNET.BizLogic
{
    ...
    public class Adm_RightsBLL : BaseBLL<Adm_Rights>
    {
        ...
        {
            List<Adm_Rights> resultList = DMST.Create<Adm_Rights>()
        ...
        public void UpdateMenuPath(int? RightsParentID, int? RightsID)
        {
            ...
            if (RightsParentID > 0)
            {
                Adm_Rights entity = DMST.Create<Adm_Rights>().Where(q => q.RightsID == RightsParentID).Select(q => q.Columns(q.MenuPath)).ToEntity();
            ...
            DMST.Create<Adm_Rights>().Edit(updateEntity, q => q.RightsID == RightsID);
        }
        ...
        {
            return DMST.Create<vw_Adm_Rights>().Where(q => q.RightsID == rightsID).ToEntity();
        }
        /// <summary>
        /// 获取职位或用户当前设置的权限
        /// </summary>
        ...
        public List<GroupRightsList> GetRigthsList(UserGroupParam param)
        {
            ...
            List<vw_Adm_Rights_User> groupList = DMST.Create<vw_Adm_Rights_User>()
        ...
        public List<GroupRightsList> GetRigthsAllList(UserGroupParam param)
        {
            ...
            List<Adm_GroupUser> groupUserList = DMST.Create<Adm_GroupUser>().Where(q => q.UserID == param.UserGroupID).Select(q => q.Columns(q.GroupID)).ToList();
